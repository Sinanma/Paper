...
The data set was collected by the city of Toronto's data @opendatatoronto. We have choose the daily shelter and overnight service occupancy and capacity as the main direction of research. We analysis the data in R(@citeR), with additional tools for support my analysis, including `tidyverse`[@citetidyverse], `here`[@citehere], `dplyr`[@citedplyr], `readr`[@citereadr], `modelsummary`[@citemodelsummary], `janitor`[@citejanitor], `tibble`[@citetibble], `ggplot2`[@citeggplot2], and research method is from @tellingstorieswithdata. 
